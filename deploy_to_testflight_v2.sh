#!/bin/bash
# Flirrt.AI Autonomous TestFlight Deployment Script v2
# Updated with -allowProvisioningUpdates for automatic profile creation
...
echo "🚀 Starting Flirrt.AI TestFlight Deployment v2..."
echo "📅 $(date)"
echo ""
...
# 2. Export IPA with automatic provisioning updates
echo ""
echo "📦 Exporting IPA from archive with automatic provisioning..."
echo "   Using -allowProvisioningUpdates to auto-create profiles"
echo ""
...
if [ $EXPORT_EXIT_CODE -ne 0 ]; then
    ...
    echo ""
    echo "💡 If authentication is required, try one of these:"
    echo "   1. Sign in to Xcode once with: office@flirrt.ai"
    echo "   2. Use fastlane: sudo gem install fastlane -NV && fastlane pilot upload"
    exit 1
fi
